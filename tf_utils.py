...
def train_op(grads, vars, global_step = None,  use_lock = True, max_clip = 40., lr = 1e-4):
    grads, _ = tf.clip_by_global_norm(t_list=grads, clip_norm=max_clip)
    return tf.group(tf.train.AdamOptimizer(learning_rate=lr, use_locking=use_lock).apply_gradients(zip(grads, vars),
